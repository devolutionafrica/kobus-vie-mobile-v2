...
var BaseSocialLogin = class extends WebPlugin {
  constructor() {
    super();
  }
  parseJwt(token) {
...
var AppleSocialLogin = class extends BaseSocialLogin {
  constructor() {
    super(...arguments);
    this.clientId = null;
  ...
  initialize(clientId, redirectUrl) {
    return __async(this, null, function* () {
      this.clientId = clientId;
      this.redirectUrl = redirectUrl || null;
      if (clientId) {
        yield this.loadAppleScript();
      }
    });
  }
  login(options) {
...
var FacebookSocialLogin = class extends BaseSocialLogin {
  constructor() {
    super(...arguments);
    this.appId = null;
    this.scriptLoaded = false;
...
var GoogleSocialLogin = class extends BaseSocialLogin {
  constructor() {
    super(...arguments);
    this.clientId = null;
  ...
  initialize(clientId, mode, hostedDomain, redirectUrl) {
    return __async(this, null, function* () {
      this.clientId = clientId;
      if (mode) {
        this.loginType = mode;
  ...
  logout() {
    return __async(this, null, function* () {
      ...
      if (!state) return;
      yield this.rawLogoutGoogle(state.accessToken);
    });
  }
  isLoggedIn() {
    return __async(this, null, function* () {
      ...
      try {
        const isValidAccessToken = yield this.accessTokenIsValid(state.accessToken);
        ...
        if (isValidAccessToken && isValidIdToken) {
          ...
          try {
            yield this.rawLogoutGoogle(state.accessToken, false);
          } catch (e) {
  ...
  rawLogoutGoogle(accessToken, tokenValid = null) {
    return __async(this, null, function* () {
      if (tokenValid === null) {
        tokenValid = yield this.accessTokenIsValid(accessToken);
      }
      if (tokenValid === true) {
        try {
          yield fetch(`https://accounts.google.com/o/oauth2/revoke?token=${encodeURIComponent(accessToken)}`);
...
//# sourceMappingURL=web-JGGMCKRB.js.map
